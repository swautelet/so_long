...
/*   Updated: 2022/03/03 16:48:08 by swautele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	animate(t_img *dt)
{
	...
	{
		usleep(ANIM_WAIT);
		mlx_put_image_to_window(dt->video, dt->win, dt->floor, dt->pos_x * dt->siz, (dt->pos_y - 1) * dt->siz);
		if (dt->pos_x == dt->pos_d_x && (dt->pos_y - 1) == dt->pos_d_y)
			mlx_put_image_to_window(dt->video, dt->win, dt->door, dt->pos_x * dt->siz, (dt->pos_y - 1) * dt->siz);
